...
config.STEALTH_LOGS = True  # False to silence [scrapy-stealth] console + package logs
...
| `STEALTH_LOGS`                | `bool`           | `True`                            | Styled `[scrapy-stealth]` console messages and package logger output. Set `False` in **`settings.py` or spider `custom_settings`** — not in `request.meta`. The PyPI update notice is always shown when a newer version exists. Readable from Scrapy settings as `STEALTH_LOGS` (applied in middleware `from_crawler` / `spider_opened`) |
